...
## Load Functions
source("C:/Users/mijke/Box/CLPN/Run on Mijke Computer/ComparingCLPNEstimatorFunctions_Nonstationary.R")
...
ResultList <- foreach(c = 1:nrow(conditions), .errorhandling = "stop") %:% 
  foreach(m = 1:20, .packages = exp_pkg) %dopar% {
  ...
  ResultsDf <- NULL
    ...
    ########################################
      ...
      SigmaList <- GenerateNonstationaryCov(BList, ResidualBank = resCovBank)
        ...
        if(any(lapply(SigmaList, is.character)== FALSE)){
          ...
          longitudinalSigmaWithin <- SigmaList$LongCov
          ...
          if (min(eigen(longitudinalSigmaWithin)$values) > 0) {break}
    ...
    for(g in 1:100){ # Generate this many samples from each population covariance matrix
      ...
      save(parameterResultsList, file = paste('C:/Users/mijke/Box/CLPN/Run on Mijke Computer/BetweenPersonStableVariance/Results/Model Results/Nonstationary/ModelResults', n, p, sparsity, model, g, '.Rdata', sep = '.'))
    ...
    save(ResultsDf, file = paste('C:/Users/mijke/Box/CLPN/Run on Mijke Computer/BetweenPersonStableVariance/Results/Condition Results/Nonstationary/ResultsDf', n, p, sparsity, model, '.Rdata', sep = '.'))
...
date <- Sys.Date()
save(ResultsDf, 
     file = paste0("C:/Users/mijke/Box/CLPN/Run on Mijke Computer/BetweenPersonStableVariance/Results/NonstationRes", 
                   date, ".RData"))
